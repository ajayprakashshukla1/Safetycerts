...
public function list_all_job($id=NULL){
    ...
    $this->db->select('users.first_name,users.last_name,properties.name,properties.unique_id as prop_unique_id,properties.status as prop_status,properties.address,assign_jobs.created_date,certificate_types.certificate_name');
    $this->db->from('assign_jobs');
	$this->db->join('users', 'assign_jobs.job_con_id = users.id');
	$this->db->join('properties', 'assign_jobs.job_prop_id = properties.id');
	$this->db->join('certificate_types', 'assign_jobs.certficate_id = certificate_types.certificate_id');
	if($id){
