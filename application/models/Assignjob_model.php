...
public function get_unique_pro_name($unique_id){
    $this->db->select('name');
    $this->db->from('properties');
    $this->db->where('unique_id', $unique_id);
    //return $this->db->get()->row()->id;
...
public function get_unique_pro_user_id($unique_id){
...
public function get_certificate_by_prop_id($id,$cert_type,$job_id=NULL){
    ...
    $this->db->select('certificate_files.*, certificate_types.certificate_name as certificate_type_name,properties.name as property_name,properties.unique_id as unique_id');
    $this->db->from('certificate_files');
    
    $this->db->join('certificate_types', 'certificate_files.certificate_type = certificate_types.certificate_id');
    $this->db->join('properties','certificate_files.certificate_property_id = properties.id');
...
public function get_other_certificatefile_by_cerunique($unique_id){
    $this->db->select('other_certificate_file');
    $this->db->from('other_certificate_files');
    $this->db->where('unique_id', $unique_id);
    return $this->db->get()->row()->other_certificate_file;
}

public function get_other_cert_by_cerunique($prop_id='',$certificate_unique_id=''){
    $this->db->select('other_certificate_file,certificate_name,unique_id');
    $this->db->from('other_certificate_files');
    $this->db->where('certificate_property_id', $prop_id);
    $this->db->where('certificate_unique_id', $certificate_unique_id);
    //$this->db->where('other_certificate_file <>', '');
    return $this->db->get()->result_array();
}

public function get_other_certificates_download($certificate_unique_id=''){
    $this->db->select('*');
    $this->db->from('other_certificate_files');
    //$this->db->where('certificate_property_id', $prop_id);
    $this->db->where('certificate_unique_id', $certificate_unique_id);
...
public function get_other_cert_by_cerunique_first($prop_id='',$cert_type=''){
...
public function get_all_parentcertificate_list($parent_arr=NULL){
    $parents= implode(",",$parent_arr);
    ...
    $this->db->join('properties', 'properties.id = certificate_files.certificate_property_id');
    $this->db->join('certificate_types', 'certificate_files.certificate_type = certificate_types.certificate_id');
    $where = "properties.user_id IN (".$parents.")";
    $this->db->where($where);
...
public function get_all_childcertificate_list($childs_arr=NULL){
    $childs= implode(",",$childs_arr);
    ...
    $this->db->join('properties', 'properties.id = certificate_files.certificate_property_id');
    $this->db->join('certificate_types', 'certificate_files.certificate_type = certificate_types.certificate_id');
    $where = "properties.user_id IN (".$childs.")";
    $this->db->where($where);
...
function status_count_parent($parent_arr = NULL) {
    $parents= implode(",", $parent_arr);
    if(empty($parents)){
       return 0; 
    }
    ...
    $this->db->from('properties');
    $condition = array();
    $where = 'status='.$status.' AND user_id IN ('.$parents.') ';  
    $this->db->where($where);  
    $count = $this->db->count_all_results();
...
function status_count_child($childs_arr = NULL) {
    $childs= implode(",", $childs_arr);
    if(empty($childs)){
       return 0; 
    }
    ...
    $this->db->from('properties');
    $condition = array();
    $where = 'status='.$status.' AND user_id IN ('.$childs.') ';  
    $this->db->where($where);  
    $count = $this->db->count_all_results();
    //echo $this->db->last_query(); exit; 
    return $count;
...
function admin_assigned_job_parent($parent_arr=NULL){
     $where = '';
     $parents= implode(",", $parent_arr);
     if(empty($parents)){
        return $result= array();
...
function admin_assigned_job_child($childs_arr=NULL){
     $where = '';
     $childs= implode(",", $childs_arr);
     if(empty($childs)){
        return $result= array();
...
function admin_assigned_list_parent($parent_arr=NULL){
    $where = '';
    $parents= implode(",", $parent_arr);
    if(empty($parents)){
       return $result= array();
...
function admin_assigned_list_child($childs_arr=NULL){
    $where = '';
    $childs= implode(",", $childs_arr);
    if(empty($childs)){
       return $result= array();
 ...
 public function get_other_certificates_row($prop_id=NULL,$cert_type=NULL){
    $this->db->select('*');
    $this->db->from('other_certificate_files');
    $this->db->where('certificate_unique_id', $prop_id);
    $this->db->where('certificate_type', $cert_type);
    $this->db->where('other_certificate_file <>', '');
