...
$(function(){
	$("#file_input").change(function(){
		...
		for (var i = 0; i < $(this).get(0).files.length; ++i) {
			...
			if($(this).prop('files')[i+1]==undefined)
				flag=1;
			...
			$.ajax({
                ...
                success: function(php_script_response){
                    ...
                    html +='<a href="#" class="close close_cert close_multiple" id='+i+'>×</a>';
					...
					var input_html="<input type=hidden name='other_certificate[]' value='"+php_script_response+"'/>";
...
$(function(){
	$(document).on('click','.close_cert',function(e){
		e.preventDefault();
		var filename=$('#file_link').text();
		var base_url="<?php echo base_url(); ?>";
		var url=base_url+"user/removeuploadjobcertificates";
		$.ajax({
                url: url, // point to server-side PHP script 
                dataType: 'text',  // what to expect back from the PHP script, if anything
                cache: false,
                contentType: false,
                processData: false,
                data: jQuery.param({ 'filename': filename}),                         
                type: 'POST',
                success: function(php_script_response){
					$("#uploaded_file").html('');
                }
