...
public function dashboard(){
  ...
  if($login_user->role=='members'){
     $id=$login_user->id;
     // get all parent
     $parents= $this->user_model->get_all_parents($login_user->id);
     $parent_arr= array();
     foreach($parents as $parent){
         $parent_arr[]=$parent['parent_id'];
     }
     
     // get all child
     $childs= $this->user_model->get_all_childs($login_user->id);

     $childs_arr= array();
     foreach($childs as $child){
         $childs_arr[]=$child['child_id'];
     }

     $data['get_all_dates'] = $this->get_all_dates();

     $all_certificate_self = $this->assignjob_model->get_allcertificate_list($id);
     $all_certificate_parent = $this->assignjob_model->get_all_parentcertificate_list($parent_arr);
     $all_certificate_child = $this->assignjob_model->get_all_childcertificate_list($childs_arr);
     // merge all cert in to single array
     $all_certificate= array_merge($all_certificate_self,$all_certificate_parent,$all_certificate_child);
     $data['all_certificate']= $all_certificate;
     //==========================================

     $data['admin_assigned_list']= $this->assignjob_model->admin_assigned_list($id);
     //print_r($data['admin_assigned_list']); exit;
  }
...
public function certificate(){
  ...
  if($this->ion_auth->is_admin()){
      ...
      $this->load->view('layout/dashboard_footer');
  }else if($login_user->role=='members'){
      $id=$login_user->id; 
...
public function parent_certificate(){
  $data['title']='Certificate';
  $data['login_user']=$this->loggedin_user;
  $login_user = $this->loggedin_user;
  ...
  if($login_user->role=='members'){
      ...
      $data['get_all_dates'] = $this->get_all_dates();
      $data['all_certificate'] = $this->assignjob_model->get_all_parentcertificate_list($parent_arr);
      
      $data['pending_status_test']=  $this->assignjob_model->status_count_parent($parent_arr);
      $data['admin_assigned_job']=  $this->assignjob_model->admin_assigned_job_parent($parent_arr);
      $data['admin_assigned_list']=  $this->assignjob_model->admin_assigned_list_parent($parent_arr);
      
      //echo "<pre>"; print_r($data['admin_assigned_list']); exit;

      $this->load->view('layout/dashboard_header',$data);
      $this->load->view('user/parent_certificate',$data);
      $this->load->view('layout/dashboard_footer');
  }else{
      redirect('auth/error_404');
  }
...
public function child_certificate(){
  $data['title']='Certificate';
  $data['login_user']=$this->loggedin_user;
  $login_user = $this->loggedin_user;
  ...
  if($login_user->role=='members'){
      ...
      $data['get_all_dates'] = $this->get_all_dates();
      $data['all_certificate'] = $this->assignjob_model->get_all_childcertificate_list($childs_arr);
      $data['pending_status_test']=  $this->assignjob_model->status_count_child($childs_arr);
      $data['admin_assigned_job']=  $this->assignjob_model->admin_assigned_job_child($childs_arr);
      $data['admin_assigned_list']=  $this->assignjob_model->admin_assigned_list_child($childs_arr);

      $this->load->view('layout/dashboard_header',$data);
      $this->load->view('user/child_certificate',$data);
      $this->load->view('layout/dashboard_footer');
  }else{
      redirect('auth/error_404');
  }
...
public function child_member($unique_id=NULL){
  $data['title']='Add Child Member';
  if(!$this->ion_auth->is_admin()){
     redirect('auth/error_404'); 
  }
  ...
  $data['user_lists'] = $this->ion_auth->users($role)->result();
  // get parent
  $parent=$this->user_model->getuserby_uniqueid($unique_id);
  $data['parent']= $parent;
  // get all member child
  $childs= $this->user_model->get_all_child($parent->id);
  $data['childs']= $childs;

  $this->load->view('layout/dashboard_header',$data);
  $this->load->view('user/child_member');
  $this->load->view('layout/dashboard_footer');

}

public function addChildMemberAjax(){
  $parent_id= $this->input->post('parent_id');
  $child_id= $this->input->post('child_id');
  // check if already exist
  $exist=$this->user_model->isChildMemberExist($parent_id,$child_id);
  if(!$exist){
      $data_arr= array();
      $data_arr['child_id'] = $child_id;
      $data_arr['parent_id'] = $parent_id;
      $data_arr['created'] = date('Y-m-d');
      if($this->db->insert('child_members', $data_arr)){
         echo "added";
      }
  }else{
     echo "child member already exist";
  }
...
public function removeChildMemberAjax(){
   $parent_id= $this->input->post('parent_id');
   $child_id= $this->input->post('child_id');

   $this->db->where('parent_id', $parent_id);
   $this->db->where('child_id', $child_id);
   $this->db->delete('child_members');
   echo "deleted";
   die();
}
  ...
  public function createproperty(){
    ...
    //----------------
      ...
      if($loggedin_user->role=='admin'){
       ...
       $email= $this->assignjob_model->get_user_email_by_user_id($post['user_id']); 
...
public function removecertificatesfiles(){
   $token= $this->input->post('token');
   $cert_type= $this->input->post('cert_type');
   $file_name= $this->input->post('file_name');
   
   $cert_file= $_SESSION['certdocfile'][$token][$cert_type]['name'];
   // unlink file from server
   $full_path= $_SERVER['DOCUMENT_ROOT']."/safetycerts/uploads/cert/".$cert_file;
   if(file_exists($full_path)){
      unlink($full_path);
   }
   
   unset($_SESSION['certdocfile'][$token][$cert_type]['name']);
   echo "deleted";
   die();
}


public function removeOthercertificatesfiles(){
   $token= $this->input->post('token');
   $cert_type= $this->input->post('cert_type');
   $other_cert_type= $this->input->post('other_cert_type');
   $file_name= $this->input->post('file_name');
   
   $full_path= $_SERVER['DOCUMENT_ROOT']."/safetycerts/uploads/cert/".$file_name;
   if(file_exists($full_path)){
      unlink($full_path);
   }

   foreach($_SESSION['certdocfile'][$token][$cert_type]['other'] as $key=>$val){
       if($val==$file_name){
          unset($_SESSION['certdocfile'][$token][$cert_type]['other'][$key]);
       }
   }
  ...
  $data['token'] = mt_rand(11111111,99999999);
    ...
    // get token
      ...
      if(isset($_SESSION['certdocfile'][$token])){
         $files= $_SESSION['certdocfile'][$token];
         unset($_SESSION['certdocfile'][$token]);
         //echo "<pre>"; print_r($_SESSION['certdocfile'][$token]); exit;
      }
...
public function delete_otherCertificateAjax(){
   $unique_id= $this->input->post('unique_id');      
	...
	public function removeuploadjobcertificates(){
		$pathfilename =  $_SERVER['DOCUMENT_ROOT']."/safetycerts/uploads/cert/".$_POST['filename'];
		unlink($pathfilename);
		exit;
	}
